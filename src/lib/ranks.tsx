...
/**
 * 龍階 Dragon Ranks, highest first. Thresholds are percent of circulating supply; `art` is the
 * sumi-e tier illustration ghosted behind each card (public/ranks, generated 2026-09-02, masters
 * in the design record). Shared by DragonRanks (the six cards), RankCard (one wallet) and the
 * /rank/<address> permalink + its Open Graph image.
 *
 * Icons: game-icons.net, CC BY 3.0 (Lorc, Delapouite).
 */
export type RankKey = 'ryujin' | 'shogun' | 'daimyo' | 'samurai' | 'ronin' | 'ashigaru';

export interface RankTier {
  key: RankKey;
  name: string; // e.g. '侍 Samurai'
  subtitle: string;
  percentage: number; // minimum percent of supply
  color: string;
  gradientFrom: string;
  gradientTo: string;
  icon: React.ReactNode;
  kanji: string;
  art: string; // public path of the ghosted tier illustration
}
...
export const RANK_TIERS: RankTier[] = [
  ...
  },
  {
    ...
    percentage: 0, // any balance: the pipeline counts every holder below 浪人 as 足軽
    key: 'ashigaru',
    art: '/ranks/ashigaru.webp',
    color: '#8D6E63',
    gradientFrom: '#8D6E63',
    gradientTo: '#5D4037',
    icon: (
...
export interface HolderRow {
  a: string; // lowercase address
  b: number; // balance in TINC
}
...
export interface HoldersFile {
  ...
  totalSupply: number;
  count: number;
  holders: HolderRow[]; // sorted by balance, descending
}

export interface RankResult {
  address: string;
  balance: number;
  pct: number; // percent of circulating supply
  rank: number; // 1-based position in the holder list
  count: number;
  totalSupply: number;
  updatedAt: string;
  tier: RankTier | null; // null when below 足軽 Ashigaru
  next: RankTier | null; // the rank above, null at 龍神 Ryūjin
  shortfall: number | null; // TINC still needed for `next`
  progress: number; // 0..1 through the current band, 1 at the top rank
}
...
export function tierForPct(pct: number): { tier: RankTier | null; next: RankTier | null } {
  for (let i = 0; i < RANK_TIERS.length; i++) {
    if (pct >= RANK_TIERS[i].percentage) {
      return { tier: RANK_TIERS[i], next: i > 0 ? RANK_TIERS[i - 1] : null };
    }
  }
  return { tier: null, next: RANK_TIERS[RANK_TIERS.length - 1] };
}

/** Where one wallet stands: tier, position, and how far the next rank is. Pure; used by the client lookup and the server rank page. */
export function rankLookup(file: HoldersFile, address: string): RankResult | null {
  const idx = file.holders.findIndex((h) => h.a === address);
  if (idx === -1) return null;
  const row = file.holders[idx];
  const supply = file.totalSupply > 0 ? file.totalSupply : 0;
  const pct = supply ? (row.b / supply) * 100 : 0;
  const { tier, next } = tierForPct(pct);
  const tierMin = tier ? (tier.percentage / 100) * supply : 0;
  const nextMin = next ? (next.percentage / 100) * supply : null;
  const shortfall = nextMin !== null ? Math.max(0, nextMin - row.b) : null;
  const progress = nextMin !== null ? Math.max(0, Math.min(1, (row.b - tierMin) / Math.max(nextMin - tierMin, 1e-9))) : 1;
  return {
    address,
    balance: row.b,
    pct,
    rank: idx + 1,
    count: file.count || file.holders.length,
    totalSupply: supply,
